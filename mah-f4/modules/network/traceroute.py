...
def traceroutet(destination_server, count_of_packets=3, packet_size=52, max_hops=64, timeout=1000):
...
class traceroute(Module):
    ...
    def run(self):
        t = Traceroute(self.config.option("target").value, self.config.option("count").value, self.config.option("packet_size").value, self.config.option("max_hops").value, self.config.option("timeout").value)
        t.start_traceroute()
